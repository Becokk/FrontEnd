  ...
  <RoadMapWrapper>
    ...
    <TimelineContainer>
      ...
      {programs.map((program) => {
        const isActive =
          !activeFilter || program.competencyName === activeFilter;
        const textColor = isActive
          ? getTextColor(program.competencyName)
          : "#B1B1B1";
        return (
          <ProgramBlock
          ...
          >
            <ProgramText style={{ color: textColor }}>
              {program.title}
            </ProgramText>
          </ProgramBlock>
        );
...
const RoadMap = () => {
  ...
  const assignRows = (programs) => {
    ...
    sorted.forEach((p) => {
      ...
      while (!placed) {
        ...
        if (
          rows[row].every(
            (ex) => e < new Date(ex.startDate) || s > new Date(ex.endDate)
          )
        ) {
          rows[row].push(p);
          placed = true;
    ...
    return sorted.map((p) => ({
      ...p,
      verticalPosition: p.row * spacing + 40,
    }));
  };
  ...
  const calculatePosition = (date) => {
    const start = new Date(
      `${now.getFullYear()}-${String(currentMonth).padStart(2, "0")}-01`
    );
